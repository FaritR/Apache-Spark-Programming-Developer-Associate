...
files = dbutils.fs.ls("FileStore/tables/adi_4578_run_400_record_DDSr_GP_AvgY_description_Insite_Read_parsetime_1657691586.parquet")
display(files)

# COMMAND ----------

dbutils.fs.rm("FileStore/tables/adi_5102_run_600_record_DDSr_GP_AvgY_description_Insite_Read_parsetime_1630355559-1.parquet")
...
# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS users_ USING parquet OPTIONS (path "/mnt/training/ecommerce/users/users.parquet");
...
# MAGIC %sql
# MAGIC SELECT ROUND(AVG(purchase_revenue_in_usd), 2) AS Average_Purchase_revenue
# MAGIC FROM sales
...
# MAGIC %sql
# MAGIC SELECT DISTINCT(event_name) AS Dist_Event_Name
# MAGIC FROM events
