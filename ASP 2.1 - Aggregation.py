...
eventCountsDF = df.groupBy("event_name").sum()
...
avgStatePurchasesDF = df.groupBy("geo.state").mean("ecommerce.purchase_revenue_in_usd")
...
statePurchasesDF = df.groupBy("geo.state").agg(var_pop("ecommerce.total_item_quantity"))
